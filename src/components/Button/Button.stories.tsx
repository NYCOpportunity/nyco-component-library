...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'The primary **action trigger** of the library. Use it for form submissions, dialog',
          'confirmations, and any interactive command. It extends every native',
          '`<button>` attribute (`onClick`, `type`, `disabled`, `aria-*`, …), so it drops into',
          'existing markup without ceremony.',
          '',
          '---',
          '',
          '## Anatomy',
          ...
          '| --- | --- | --- | --- |',
          '| Label | `children` | — | Button text. Omitted when `iconOnly`. |',
          '| Start icon | `startIcon` | — | Node rendered before the label. |',
          '| End icon | `endIcon` | — | Node rendered after the label. |',
          '| Icon-only | `iconOnly` | — | Square button showing a single icon; hide the text label. |',
          '',
          '## How to use it',
          '',
          '```tsx',
          ...
          '',
          '// Secondary, small, with a trailing icon',
          '<Button variant="secondary" size="small" endIcon={<ArrowIcon />}>',
          '  Learn more',
          '</Button>',
          '',
          '// Icon-only',
          '<Button iconOnly aria-label="Close" startIcon={<CloseIcon />} />',
          '```',
          '',
          '## Variants',
          '',
          ...
          '| --- | --- | --- |',
          '| `primary` *(default)* | Filled brand background, white text | The main action on a view. |',
          '| `secondary` | White background with a border | Secondary / neutral actions. |',
          '| `text` | Link-styled, no fill; underlines on hover | Low-emphasis inline actions. |',
          '',
          '## Sizes',
          '',
          '`size="large"` *(default)* and `size="small"`. Large buttons scale their padding and',
          'font down automatically below 1000 px so touch targets stay comfortable on mobile.',
          '',
          '## States',
          '',
          '- **Hover** — background (primary/secondary) or underline (text) shifts on pointer',
          '  devices; on touch devices a brief tap "flash" gives the same feedback.',
          '- **Disabled** — set `disabled` to dim the button and block pointer events.',
          '',
          '## Accessibility',
          '',
          '- Renders a real `<button>` with `type="button"` by default (override via `type`).',
          '- Shows a visible focus ring (`focus-visible`) for keyboard users.',
          '- When using `iconOnly`, always pass an `aria-label` so the action has an accessible name.',
        ].join('\n'),
      },
    },
  },
