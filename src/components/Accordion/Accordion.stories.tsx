...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'An expandable / collapsible panel with two variants: **Accordion** for standard content',
          'panels, and **In-Page Nav** for a Table of Contents. A single header toggles the panel',
          'open and closed, revealing either arbitrary children or a list of navigation items.',
          '',
          '---',
          '',
          '## Anatomy',
          ...
          '| --- | --- | --- | --- |',
          '| Title | `title` | ✅ | Header label shown in the trigger button. |',
          '| Variant | `variant` | — | `"accordion"` (default) or `"in-page-nav"`. |',
          '| Body | `children` | — | Content shown when `variant="accordion"` and open. |',
          '| Nav items | `items` | — | Array of `AccordionNavItem` (`label`, optional `href`, `active`, `onClick`) shown when `variant="in-page-nav"` and open. |',
          ...
          '',
          '```tsx',
          'import { Accordion } from "@nycopportunity/component-library";',
          '',
          '// Standard content panel',
          '<Accordion title="Eligibility requirements" defaultOpen>',
          '  <p>You must be a NYC resident aged 18 or older.</p>',
          '</Accordion>',
          '',
          ...
          '## Variants',
          '',
          '- **`accordion`** — the open panel renders `children` as body content in a `role="region"`.',
          '- **`in-page-nav`** — the open panel renders `items` inside a `<nav>`. Each item shows an',
          '  accent bar (dark when `active`, light otherwise) and renders as an `<a>` when `href` is set,',
          '  otherwise as plain text.',
          '',
          '## Behavior',
          '',
          'The panel can be **controlled** (pass `open` + `onOpenChange`) or **uncontrolled** (pass',
          '`defaultOpen`). The header `+` icon rotates 45° to an `×` when open. Setting `disabled`',
          'prevents toggling and dims the whole component to 40% opacity.',
          '',
          '## Accessibility',
          '',
          '- The header is a real `<button>` exposing `aria-expanded` and `aria-controls` targeting the panel.',
          '- It receives a 3 px inset focus ring on keyboard focus and is fully keyboard-operable.',
          '- `variant="accordion"` panels use `role="region"` labelled by the title; `variant="in-page-nav"`',
          '  panels use a `<nav>` labelled by the title.',
          '- The `disabled` state sets the native `disabled` attribute and blocks pointer interaction.',
        ].join('\n'),
      },
    },
  },
