...
const meta: Meta<typeof Chip> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'A compact, interactive **chip** used to represent a filter, tag, or attribute. Use it',
          'to let people toggle a facet on and off, or to display an active selection that can be',
          'removed. For managing a whole row of chips together, reach for **ChipGroup**.',
          '',
          '---',
          '',
          '## Anatomy',
          ...
          '| --- | --- | --- | --- |',
          '| Label | `label` | ✅ | Text shown inside the chip. |',
          '| Info icon + tooltip | `tooltip`, `tooltipTitle` | — | Renders an ⓘ icon; shows a tooltip on hover / focus. Selectable variant only. |',
          '| Dismiss icon | — | — | The × shown by the `dismissible` variant. Visual only — the whole chip is the trigger. |',
          '',
          '---',
          '',
          '## How to use it',
          '',
          '```tsx',
          'import { Chip } from "@nycopportunity/component-library";',
          '',
          '// Selectable — toggles a highlighted state',
          '<Chip',
          '  label="Poverty rate"',
          '  tooltip="Share of residents below the poverty line."',
          '  defaultSelected',
          '  onSelectedChange={(selected) => console.log(selected)}',
          '/>',
          '',
          '// Dismissible — the whole chip removes itself',
          '<Chip variant="dismissible" label="Brooklyn" onDismiss={() => remove("brooklyn")} />',
          '```',
          '',
          '---',
          '',
          '## Variants',
          '',
          '`variant="selectable"` (default) — a `role="checkbox"` button that toggles a',
          'highlighted/selected state on each click. Selection can be controlled (`selected` +',
          '`onSelectedChange`) or uncontrolled (`defaultSelected`). Supports an optional `tooltip`.',
          '',
          '`variant="dismissible"` — shows an × icon; the entire chip surface is the dismiss trigger',
          'and calls `onDismiss` when clicked. Does not support selection or tooltips.',
          '',
          '## States',
          '',
          'Default, hover, selected, selected-hover, focus, and disabled. `disabled` dims the chip',
          'to 30% opacity and blocks all pointer interaction.',
          '',
          '## Accessibility',
          '',
          '- Selectable chips are real `<button>`s with `role="checkbox"` and `aria-checked`',
          '  reflecting the selected state.',
          '- Dismissible chips expose an `aria-label` of `"Remove {label}"`; the × icon is',
          '  `aria-hidden`.',
          '- Fully keyboard operable — focusable, activated with Enter / Space, with a visible',
          '  `focus-visible` ring.',
        ].join('\n'),
      },
    },
